...
function cmdSet(root, args) {
  if (args.length < 3) fatal('usage: backlog set <id> <field> \'<json>\'');
  const id = normalizeId(args[0]);
  const field = args[1];
  const allowed = ['design', 'implementation', 'review', 'testing'];
  ...
  console.log(JSON.stringify({ id, field, ok: true }));
}
...
Commands:
  ...
  set <id> <field> '<json>'                    Set design/implementation/review/testing
